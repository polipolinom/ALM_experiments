...
import wandb
import utils
from models import Encoder, ModelPrior, RewardPrior, Discriminator, Critic, Actor, Identity
...
class AlmAgent(object):
    def __init__(self, device, action_low, action_high, num_states, num_actions,
                ...
                use_kl_loss, use_classifier_loss, use_value_loss,
    ...
    def _intrinsic_reward_loss(self, z, action_batch, z_next, z_next_prior, log, metrics):
        ...
        if not self.use_classifier_loss:
            classifier_loss = torch.zeros_like(classifier_loss)
    ...
    def _init_networks(self, num_states, num_actions, latent_dims, hidden_dims, model_hidden_dims):
        ...
        self.encoder = Identity() if self.use_identity_encoder else Encoder(num_states, hidden_dims, latent_dims).to(self.device)
        self.encoder_target = Identity() if self.use_identity_encoder else Encoder(num_states, hidden_dims, latent_dims).to(self.device)

...
def test_agent(agent, test_env, max_ep_len, logger, n_eval=1):
    ...
    eval_metrics = dict()
    eval_metrics['eval_episodic_return'] = 0
    eval_metrics['eval_episodic_length'] = 0
    for j in range(n_eval):
        ...
        while not (d or (ep_len == max_ep_len)):
            ...
            ep_ret += r
            ep_len += 1
            eval_metrics['eval_episodic_return'] += r
            eval_metrics['eval_episodic_length'] += 1
        ep_return_list[j] = ep_ret
        if logger is not None:
            logger.store(TestEpRet=ep_ret, TestEpLen=ep_len)
    
    eval_metrics['eval_episodic_return'] /= n_eval
    eval_metrics['eval_episodic_length'] /= n_eval
    return ep_return_list, eval_metrics

...
class REDQSACAgent(object):
    ...
    def get_save_dict(self):
        save_dict = {}
        save_dict['policy_net'] = self.policy_net.state_dict()

        for i in range(len(self.q_net_list)):
            save_dict[f'q_net_{i}'] = self.q_net_list[i].state_dict()
            save_dict[f'q_target_net_{i}'] = self.q_target_net_list[i].state_dict()

...
def redq_sac(env_name, seed=0, epochs='mbpo', steps_per_epoch=1000,
             ...
             hidden_sizes=(256, 256), replay_size=int(1e6), batch_size=512,
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            episode_metrics['episodic_length'] = ep_len
            episode_metrics['episodic_return'] = ep_ret / ep_len
            episode_metrics['steps_per_second'] = ep_len / (time.time() - episode_start_time)
            episode_metrics['env_buffer_length'] = agent.replay_buffer.size
            wandb.log(episode_metrics, step=t)
        ...
        if t % 1000 == 0:
            snapshot = Path('./checkpoints') / Path(str(t)+'.pt')
            print(snapshot)
            save_dict = agent.get_save_dict()
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            _, eval_metrics = test_agent(agent, test_env, max_ep_len, logger, n_eval=5)
            wandb.log(eval_metrics, step=t) # add logging here
            if evaluate_bias:
...
if __name__ == '__main__':
    ...
    wandb.init(project='alm_HalfCheetah-v2', settings=wandb.Settings(start_method="thread"))
